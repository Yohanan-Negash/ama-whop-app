...
const mockApprovedQuestions = [
  ...
  {
    id: "103",
    content: "Do you ever feel like you're just copying what other successful people do instead of being original?",
  ...
  {
    id: "105",
    content: "What's the most money you've lost on a business venture that you never talk about publicly?",
...
export default function ApprovedQuestions({ withScrollArea }: ApprovedQuestionsProps) {
  const [approvedQuestions, setApprovedQuestions] = useState(mockApprovedQuestions)
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      setApprovedQuestions((prev) => prev.filter((q) => q.id !== questionToDelete))
  ...
  async function handlePushToForums(id: string) {
    ...
    try {
      ...
      setApprovedQuestions((prev) => prev.map((q) => (q.id === id ? { ...q, pushedToForum: true } : q)))
      toast({
        title: "Pushed to forums ✅",
        description: "The question has been pushed to Whop forums.",
      })
    } catch (error) {
      toast({
        title: "Error",
        description: "There was an error pushing to forums. Please try again.",
        variant: "destructive",
      })
    } finally {
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-1 ml-4 self-center">
                ...
                <Button
                  ...
                  disabled={processingIds.includes(question.id) || question.pushedToForum}
                  className={`bg-orange-500 hover:bg-orange-600 text-white rounded px-3 py-2 text-sm font-medium transition-colors opacity-70 hover:opacity-100 ${
                    question.pushedToForum ? "bg-gray-200 text-gray-500 cursor-not-allowed" : ""
                  }`}
                >
                  {question.pushedToForum ? "Posted to Forums" : "Create Forum Post"}
                </Button>
              </div>
            </div>
          </CardContent>
